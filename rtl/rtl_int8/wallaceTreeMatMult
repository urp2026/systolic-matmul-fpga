(* use_dsp = "no" *)
module array_multiplier_signed #(parameter W = 8) (
    ...
    wire [2*W-1:0] s2 = csa_sum(pp[6],pp[7],0),      c2 = csa_car(pp[6],pp[7],0);
    wire [2*W-1:0] s3 = csa_sum(s0,c0,s1),           c3 = csa_car(s0,c0,s1);
    wire [2*W-1:0] s4 = csa_sum(c1,s2,c2),           c4 = csa_car(c1,s2,c2);
    wire [2*W-1:0] s5 = csa_sum(s3,c3,s4),           c5 = csa_car(s3,c3,s4);
    wire [2*W-1:0] s6 = csa_sum(s5,c5,c4),           c6 = csa_car(s5,c5,c4);
...
module systolic_array_ws #(parameter SIZE = 4, DW = 8, AW = 32) (
    input wire clk,
    input wire rst,
    input wire load_w,
    input wire en,
...
module matmul_top_ws #(parameter SIZE = 4, DW = 8, AW = 32) (
    input wire clk,
    input wire rst,
    input wire start,
